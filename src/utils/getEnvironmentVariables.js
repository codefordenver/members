const envVars = {
  ...
  auth0ApiIdentifier: process.env.REACT_APP_AUTH0_API_IDENTIFIER || 'http://localhost:3000',
...
function makeSureVarsAreDefined() {
  ...
  Object.keys(requiredEnvNameMapping).forEach((key) => {
    if (!envVars[key]) {
      let errorMessage = `You need to export a ${requiredEnvNameMapping[key]} variable`;
